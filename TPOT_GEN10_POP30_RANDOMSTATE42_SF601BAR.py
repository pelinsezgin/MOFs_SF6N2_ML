...
# Load data
tpot_data = pd.read_excel('SF6_01BAR.xlsx', dtype=np.float64)
features = tpot_data.drop('SF6 Uptake 0.1 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = train_test_split(
    features, tpot_data['SF6 Uptake 0.1 bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Train model
exported_pipeline = RandomForestRegressor(bootstrap=False, max_features=0.35, min_samples_leaf=3, min_samples_split=6, n_estimators=100, random_state=42)
...
# Print metrics
print('R2_Train: {:.3f}'.format(r2_score(training_target, y_pred_train)))
print('R2_Test: {:.3f}'.format(r2_score(testing_target, preds)))
print('MSE_Train: {:.10f}'.format(mean_squared_error(training_target, y_pred_train)))
print('MSE_Test: {:.10f}'.format(mean_squared_error(testing_target, preds)))
print('MAE_Train: {:.10f}'.format(mean_absolute_error(training_target, y_pred_train)))
print('MAE_Test: {:.10f}'.format(mean_absolute_error(testing_target, preds)))
print('RMSE_Train: {:.7f}'.format(rmse_train))
print('RMSE_Test: {:.7f}'.format(rmse_test))
coef1, p = spearmanr(training_target, y_pred_train)
coef2, p = spearmanr(testing_target, preds)
print('SRCC_Train: {:.3f}'.format(coef1))
print('SRCC_Test: {:.3f}'.format(coef2))
...
scale_min = -0.1
scale_max = 3.5  # Ensure the same range for both axes
...
plt.text(0.02, 0.98, '(a) 0.1 bar ', transform=g.ax_joint.transAxes, fontsize=23, verticalalignment='top')
