...
features = tpot_data.drop('N2 Uptake 0.1 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['N2 Uptake 0.1 bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -6.703431002504857e-05
exported_pipeline = make_pipeline(
    RobustScaler(),
    StackingEstimator(estimator=RidgeCV()),
    RandomForestRegressor(bootstrap=True, max_features=1.0, min_samples_leaf=4, min_samples_split=5, n_estimators=100)
)
...
g.set_axis_labels(r'Simulated N$_{N_2}$ (mol/kg)', r'Predicted N$_{N_2}$ (mol/kg)', fontsize=27)

# Set both axes to the same scale
scale_min = -0.005
scale_max = 0.267  # Ensure the same range for both axes
