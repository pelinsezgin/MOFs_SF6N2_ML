...
tpot_data = pd.read_excel('logSEL_ML.xlsx', dtype=np.float64)
features = tpot_data.drop('logSEL', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['logSEL'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.03250301027630995
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=RidgeCV()),
    RandomForestRegressor(bootstrap=False, max_features=0.6000000000000001, min_samples_leaf=3, min_samples_split=6, n_estimators=100)
)
...
g.set_axis_labels(r'Simulated logS$_{SF_6/N_2}$ ', r'Predicted logS$_{SF_6/N_2}$ ', fontsize=28)
...
plt.text(0.02, 0.98, '(a)  ', transform=g.ax_joint.transAxes, fontsize=23, verticalalignment='top')
