...
features = tpot_data.drop('N2 Uptake 1 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['N2 Uptake 1 bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.002568006935203075
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=RidgeCV()),
    MinMaxScaler(),
    RandomForestRegressor(bootstrap=False, max_features=0.6000000000000001, min_samples_leaf=4, min_samples_split=6, n_estimators=100)
)
...
scale_min = -0.02
scale_max = 1.0   # Ensure the same range for both axes
