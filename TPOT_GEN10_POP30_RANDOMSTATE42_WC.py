...
tpot_data = pd.read_excel('WC_ML.xlsx',  dtype=np.float64)
features = tpot_data.drop('SF6 Working Capacity (mol/kg) VSA', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['SF6 Working Capacity (mol/kg) VSA'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.25882062451230503
exported_pipeline = RandomForestRegressor(bootstrap=False, max_features=0.35000000000000003, min_samples_leaf=3, min_samples_split=6, n_estimators=100)
...
g.set_axis_labels(r'Simulated ΔN$_{SF_6} (mol/kg) $ ', r'Predicted ΔN$_{SF_6}  (mol/kg)$ ', fontsize=28)

# Set both axes to the same scale
scale_min = -0.2
scale_max = 6.7   # Ensure the same range for both axes
...
g.ax_joint.tick_params(axis='both', which='major', labelsize=24)
...
plt.text(0.02, 0.98, '(b)  ', transform=g.ax_joint.transAxes, fontsize=23, verticalalignment='top')
