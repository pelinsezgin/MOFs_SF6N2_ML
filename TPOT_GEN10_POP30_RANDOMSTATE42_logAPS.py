...
tpot_data = pd.read_excel('logAPS_ML.xlsx', dtype=np.float64)
features = tpot_data.drop('logAPS', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['logAPS'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.09640511645193461
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=RidgeCV()),
    RandomForestRegressor(bootstrap=False, max_features=0.6000000000000001, min_samples_leaf=3, min_samples_split=2, n_estimators=100)
)
...
g.set_axis_labels(r'Simulated logAPS (mol/kg)  ', r'Predicted logAPS  (mol/kg) ', fontsize=28)

# Set both axes to the same scale
scale_min = -2.2
scale_max = 4.0   # Ensure the same range for both axes
...
plt.text(0.02, 0.98, '(c)  ', transform=g.ax_joint.transAxes, fontsize=23, verticalalignment='top')
