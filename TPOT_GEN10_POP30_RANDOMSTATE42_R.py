...
tpot_data = pd.read_excel('R_ML.xlsx',  dtype=np.float64)
features = tpot_data.drop('R% VSA', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['R% VSA'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -41.88097323330768
exported_pipeline = make_pipeline(
    PolynomialFeatures(degree=2, include_bias=False, interaction_only=False),
    RandomForestRegressor(bootstrap=False, max_features=0.2, min_samples_leaf=1, min_samples_split=15, n_estimators=100)
)
...
g.set_axis_labels(r'Simulated R%  ', r'Predicted R% ', fontsize=28)

# Set both axes to the same scale
scale_min = 0
scale_max = 100   # Ensure the same range for both axes
...
plt.text(0.02, 0.98, '(d)  ', transform=g.ax_joint.transAxes, fontsize=23, verticalalignment='top')
